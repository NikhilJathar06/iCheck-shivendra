...
router.post("/communicationlog", async (req, res) => {
    ...
    await Communicationlog.findOneAndUpdate(filter, update, options)
      .then((doc) => {
        console.log(doc)
        io.emit('communicationAdded', { message: 'New communication added' });
...
router.post('/createVesselInspection', async (req, res) => {
    ...
    data.save().then((_) => {
        io.emit('vesselInspectionAdded', { message: 'New checklist added' });
...
router.post('/createNotification', async (req, res) => {
    ...
    await notification.save().then((_) => {
        io.emit('notificationAdded', { message: 'New notification added' });
...
router.get('/updateNotiReadOfCommunication/:companyName/:checklistId/:questionNo', async (req, res) => {
    try {
        const { companyName, email, checklistId, questionNo } = req.params;
        const noti = await Notification.find({ companyName, checklistId, questionNo, notificationType: "New Communication" });
        if (noti.length === 0) {
            return res.status(400).send({ message: 'No notifications log found for this user' });
        }

        const result = await Notification.updateOne(
            { companyName, email, isReadByCompany: false, notificationType: 'New Communication' },
            { $set: { isReadByCompany: true } }
        );
  ...
  router.post('/updateLastUpdatedVesselInspection/:email', async (req, res) => {
    try{
